...
function nextChar(c) {
  ...
  function gen_ouput(){
  	var input_value = $('input[name="txtInput"]').val();
  
  var input_type = $('select[name="alpha_num"]').val();
  
  var prev_int_values = $('input[name="txtPrevIntValues"]').val();

  var prevIntValueArray = new Array();
  var last_int_value = '0';

  if(prev_int_values !== ''){
	  prevIntValueArray = prev_int_values.split(",");
	  last_int_value = (prevIntValueArray.length ? prevIntValueArray[(prevIntValueArray.length)-1] : '0');	  
  }

  var prev_apha_values = $('input[name="txtPrevAlphaValues"]').val();
  var prevAlphaValueArray = new Array();

   var last_alpha_value = '';

  if(prev_apha_values !== ''){
	  prevAlphaValueArray = prev_apha_values.split(",");
	  last_alpha_value = (prevAlphaValueArray.length ? prevAlphaValueArray[(prevAlphaValueArray.length)-1] : '');
  }
  ...
  if(input_type == 'integer'){
    ...
    var incr_int_value = ++last_int_value;
    
    output = input_value + '.' + (incr_int_value);
    
    $('input[name="txtPrevIntValues"]').val(prev_int_values + (prev_int_values !== '' ? "," : '') + incr_int_value);
    
  }else if(input_type == 'alphabet'){
	var incr_alpha_value = (last_alpha_value !== '' ? nextChar(last_alpha_value) : 'a');
    
    output = input_value + '.' + (incr_alpha_value);
    
    $('input[name="txtPrevAlphaValues"]').val(prev_apha_values + (prev_apha_values !== '' ? "," : '') + incr_alpha_value);
  }
  
  $('#dis_output').html(output);
...
<!--               <input type="radio" name="external" value="Yes"  id="id_yesforexternal">Yes&nbsp;&nbsp;&nbsp;
 ...
 -->               
         ...
         <tr class="row2">
            ...
            <td valign="middle" align="left" class="input_txt" width="25%">Issuer:</td>
              ...
              <tr class="row2" style="border:none;">
               ...
               <td valign="top" align="left" class="input_txt" width="20%">
              <!--  <input type="text" class="inputtxtbx1" name="revision_prefix" value="" class="input_txtbx145" style="height:22px;background:lightgrey;width:50px; margin:0 0 0 0; border:none;" name="revision_prefix" onblur="change_to_label();"/>
               <input type="text" class="inputtxtbx1" name="revision_prefix" value="" class="input_txtbx145" style="height:22px;background:lightgrey;width:50px; margin:0 0 0 0; border:none;" name="revision_prefix" onblur="change_to_label();"/>
               
                --><input type="text" name="txtInput"  class="input_txtbx145" style="height:22px;background:lightgrey;width:50px; margin:0 0 0 0; border:none;"  value="" onblur="gen_ouput();"/>
               <select name="revision_level" id="revisionlevel" class="input_cmbbx1" style="width:80px;height:22px;border:none;beckground:lightgrey;">
   ...
   <input type="text" name="txtPrevIntValues" value="" onclick="gen_ouput();">
   <input type="text" name="txtPrevAlphaValues" value="" onclick="gen_ouput();">
   <div id="dis_output"onclick="gen_ouput();"></div></td>         
